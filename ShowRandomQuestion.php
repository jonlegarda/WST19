...
// Taula bat definituko dugu;
	...
	if(isset($_SESSION["unekoGaldera2"])){
		$_SESSION["unekoGaldera2"]=$_SESSION["unekoGaldera2"]+1;
	}
	else{
		$_SESSION["unekoGaldera2"]=0;
	}
	//print_r($_SESSION["zenbakiak"]);
	$emaitza = $connection->query($SQL_QUIZ);
	//uneko galdera bektoretik ateratzen ez dela egiaztatu.
	//echo $_SESSION["unekoGaldera"];
	if($_SESSION["unekoGaldera2"]<$emaitza->num_rows){
		if ($emaitza->num_rows > 0) {
			while ($row = $emaitza->fetch_assoc()) {
					if($row['ID']==$_SESSION["zenbakiak"][$_SESSION["unekoGaldera2"]]){
